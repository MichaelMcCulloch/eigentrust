...
def test_should_handle_zero_columns():
    ...
    matrix = torch.tensor([[1.0, 0.0, 3.0], [2.0, 0.0, 6.0], [3.0, 0.0, 9.0]], dtype=torch.float32)
...
def test_should_preserve_zero_values():
    ...
    matrix = torch.tensor([[0.0, 2.0, 0.0], [4.0, 0.0, 6.0], [0.0, 8.0, 0.0]], dtype=torch.float32)
...
def test_should_handle_already_normalized_matrix():
    ...
    matrix = torch.tensor([[0.2, 0.5, 0.1], [0.3, 0.3, 0.4], [0.5, 0.2, 0.5]], dtype=torch.float32)
